#define SCREEN_WIDTH 640
#define SCREEN_HEIGHT 480
#define GRID_SIZE 20

#define GRID_HEIGHT (SCREEN_HEIGHT-BUTTON_HEIGHT)

#define CELL_SIZE ((int)(GRID_HEIGHT / GRID_SIZE))

#define _H ((SCREEN_WIDTH/(CELL_SIZE_CONST-1))+1)
#define _V ((GRID_HEIGHT/(CELL_SIZE_CONST-1))+1)


#define CELL_SIZE_CONST 20
#define NUMBER_OF_BUTTONS 2

#define BUTTON_ROW_START (SCREEN_HEIGHT-100)
#define BUTTON_WIDTH ((int)(SCREEN_WIDTH/NUMBER_OF_BUTTONS))
#define BUTTON_HEIGHT 100

#define BORDER_WIDTH 2

#define RED  255, 0, 0, 255     // Red color
#define GREEN  0, 255, 0, 255   // Green color
#define BLUE  0, 0, 255, 255    // Blue color

#define GREY 128,128,128,255	//GreyButton
#define GREY_PRESSED 64,64,64,255	//when button is pressed


#define BLACK 0,0,0,255			//White
#define WHITE 255,255,255,255	//Black

#define TRUE 1U
#define FALSE 0U